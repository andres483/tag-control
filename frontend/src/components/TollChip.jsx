  ...
  return (
    <div className="flex items-center justify-between bg-surface-secondary rounded-2xl px-4 py-3.5">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${inferred ? 'bg-text-tertiary/20' : 'bg-primary/10'}`}>
        ...
        <div className="min-w-0">
          <p className="text-[14px] font-medium text-text truncate">{toll.nombre}</p>
          <p className="text-[12px] text-text-tertiary">
            {toll.ruta} &middot; {formatTime(timestamp)}
            {inferred && <span className="text-text-tertiary"> &middot; zona sin GPS</span>}
          </p>
        </div>
      </div>
      <span className="text-[15px] font-semibold text-primary shrink-0 ml-2">{formatCLP(getTarifa(toll, new Date(timestamp)))}</span>
    </div>
  );
}
